##############################################################################
# backend/prod.hcl – Partial S3 backend config for the prod environment
# Used as: terraform init -reconfigure -backend-config=backend/prod.hcl
#
# The state bucket lives in the prod AWS account (GitHub variable
# AWS_ACCOUNT_ID_PROD). The pipeline reaches it by assuming an IAM role in that
...
#   s3:GetObject / PutObject / DeleteObject on arn:aws:s3:::<bucket>/auth0/prod/*
#   dynamodb:GetItem / PutItem / DeleteItem / DescribeTable on the lock table
##############################################################################

bucket = "olb-iam-tfstate-prod"
