##############################################################################
# backend/staging.hcl – Partial S3 backend config for the staging environment
# Used as: terraform init -reconfigure -backend-config=backend/staging.hcl
#
# The state bucket lives in the staging AWS account (GitHub variable
# AWS_ACCOUNT_ID_STAGING). The pipeline reaches it by assuming an IAM role in that
...
#   s3:GetObject / PutObject / DeleteObject on arn:aws:s3:::<bucket>/auth0/staging/*
#   dynamodb:GetItem / PutItem / DeleteItem / DescribeTable on the lock table
##############################################################################

bucket = "olb-iam-tfstate-staging"
