##############################################################################
# backend/qa.hcl – Partial S3 backend config for the qa environment
# Used as: terraform init -reconfigure -backend-config=backend/qa.hcl
#
# The state bucket lives in the qa AWS account (GitHub variable
# AWS_ACCOUNT_ID_QA). The pipeline reaches it by assuming an IAM role in that
...
#   s3:GetObject / PutObject / DeleteObject on arn:aws:s3:::<bucket>/auth0/qa/*
#   dynamodb:GetItem / PutItem / DeleteItem / DescribeTable on the lock table
##############################################################################

bucket = "olb-iam-tfstate-qa"
